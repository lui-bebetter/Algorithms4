 ...
 *Author:   luibebetter
 *Date:     2018/3/515:01
 *Description:The TrieST class represents an symbol table of key-value pairs,
  with string keys and generic values. It supports the usual put, get, contains,
  delete, size, and is-empty methods. It also provides character-based methods
  for finding the string in the symbol table that is the longest prefix of a
  given prefix, finding all strings in the symbol table that start with a given
  prefix, and finding all strings in the symbol table that match a given pattern.
  ...
  replace the old value with the new value. Unlike Map, this class uses the
  convention that values cannot be null—setting the value associated with a key
 ...
 *@author: luibebetter
 *@create: 2018/3/5
 *Description:This implementation uses a 256-way trie. The put, contains, delete, and
  longest prefix operations take time proportional to the length of the key (in the
  worst case). Construction takes constant time. The size, and is-empty operations
  take constant time.
 *************************************************************************************/
public class TrieST<Value> {
	...
	private Node put(Node node,String key, Value value, int d){
		/*****************************************************************
		 * Description: add the key-value pair to the subtree rooted at{@code node}
		 *
	...
	//get the value associated to {@code key}
	...
	private void collect(Node node, String pattern, StringBuilder tmp, LinkedQueue<String> q){
		...
		int d=tmp.length();
		if(d==pattern.length()) {
			if(node.value!=null) q.enqueue(tmp.toString());
			return;
		}
